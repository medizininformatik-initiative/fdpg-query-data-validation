...
do
  curl -vX POST -d @"$file" -H "Content-Type: application/json" "${BLAZE_SERVER_URL}/StructureDefintion"
  echo "Uploading ${file}"
done
for file in -exec $(find ./fhir_profiles -name '*.xml')
do
  curl -vX POST -d @"$file" -H "Content-Type: application/xml" "${BLAZE_SERVER_URL}/StructureDefintion"
  echo "Uploading ${file}"
done
