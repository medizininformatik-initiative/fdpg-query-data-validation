...
python main.py "${FHIR_SERVER_URL}" -u "${FHIR_USERNAME}" -p "${FHIR_PASSWORD}" -ft "${FHIR_TOKEN}" --http-proxy "${HTTP_PROXY}" --https-proxy "${HTTPS_PROXY}" --cert "${REQUESTS_CA_BUNDLE}" -t "${TOTAL}" -c "${COUNT}" -v "${VALIDATION_URL}"